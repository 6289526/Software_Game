...
void TimeImg::DrawTime(int time, FloatRect dst){
    ...
    tmpdst = {(float)dst.x + (float)dst.w / 7 * 6.0f, (float)dst.y, (float)dst.w / 7, (float)dst.h};
    str_s.Draw(&tmpdst);
...
void TimerUI::Set(TTF_Font *font){
    ...
    dst = {
            Wd_Width * 36 / 40 - UI_MAGNIFICATION * base.GetImgRect().w / 2,
            Wd_Height * 5 / 30 - UI_MAGNIFICATION * base.GetImgRect().h / 2,
            UI_MAGNIFICATION * base.GetImgRect().w,
            UI_MAGNIFICATION * base.GetImgRect().h
        };
    timeDst = {
            Wd_Width  * 36 / 40 - UI_MAGNIFICATION * (float)timeMagW * base.GetImgRect().w / 2,
            Wd_Height * 16 / 90 - UI_MAGNIFICATION * (float)timeMagH * base.GetImgRect().h / 2,
            UI_MAGNIFICATION * (float)timeMagW * base.GetImgRect().w,
            UI_MAGNIFICATION * (float)timeMagH * base.GetImgRect().h
        };
}
...
void PlayerNameUI::Set(const PlayerData *pData, TTF_Font *font){
    ...
    nameDst = {
            Wd_Width * 71 / 80 - UI_MAGNIFICATION * (float)nameMagW * base.GetImgRect().w / 2,
            Wd_Height * 51 / 60 - UI_MAGNIFICATION * (float)NameMagH * base.GetImgRect().h / 2,
            UI_MAGNIFICATION * (float)nameMagW * base.GetImgRect().w,
            UI_MAGNIFICATION * (float)NameMagH * base.GetImgRect().h
        };
    idDst = {
            Wd_Width * 307 / 320 - UI_MAGNIFICATION * (float)idMagW * base.GetImgRect().w / 2,
            Wd_Height * 87 / 120 - UI_MAGNIFICATION * (float)idMagH * base.GetImgRect().h / 2,
            UI_MAGNIFICATION * (float)idMagW * base.GetImgRect().w,
            UI_MAGNIFICATION * (float)idMagH * base.GetImgRect().h
        };
}
...
void DirectionUI::Draw(float playerdir, float rotdir){
    ...
    FloatRect meterDst = {dst.x, dst.y + dst.h * (1 - 0.3f * (float)sin(5 * rotdir) * (float)sin(5 * rotdir)), dst.w, dst.h};
    base.Draw(&dst);
